...
## Dynamic Progress Showcase

```tsx
import { useState } from 'react';
import { Progress, Button } from 'forteui-core';

function DynamicProgressShowcase() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <Button onClick={startProgress} disabled={progress === 100}>
          Start
        </Button>
        <Button onClick={stopProgress} variant="outline-default">
          Stop
        </Button>
      </div>
      ...
      <div className="flex justify-between text-sm text-gray-500">
        ...
        {progress === 100 && <span className="text-green-500">Completed!</span>}
      </div>
    </div>
  );
}
